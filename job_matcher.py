...
if 'easy_applied_jobs' not in st.session_state:
    st.session_state.easy_applied_jobs = []
        ...
        with tab1:
            ...
            if matches:
                for idx, (job, score) in enumerate(matches):
                    ...
                    st.markdown(f"🗓 {post_date}")

                    cols = st.columns([2, 2, 1])
                    with cols[0]:
                        if st.button(f"⚡ Easy Apply #{idx}"):
                            if job not in st.session_state.easy_applied_jobs:
                                st.session_state.easy_applied_jobs.append(job)
                                st.success(f"✅ Easy applied to: {title}")
                    with cols[1]:
                        st.markdown(f"[🌐 Apply for this Job]({url})", unsafe_allow_html=True)
                    with cols[2]:
